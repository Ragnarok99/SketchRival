...
class OpenAIService {
     ...
     */
    formatImageForOpenAI(imageDataUrl) {
        ...
        if (estimatedSize > 1000000) {
            // Si es mayor a ~1MB
            console.warn(`Imagen grande (aprox. ${Math.round(estimatedSize / 1024)}KB). Considerar redimensionar en el frontend.`);
        }
        return imageDataUrl;
    }
    /**
    ...
    evaluateDrawingWithOpenAI(imageDataUrl, wordToGuess) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const imageHash = crypto_1.default.createHash('sha256').update(formattedImage).digest('hex');
