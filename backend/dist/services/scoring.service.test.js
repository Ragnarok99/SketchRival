...
describe('calculateBaseScore', () => {
    ...
    it('debe sumar puntos por acierto', () => {
        const evento = { type: 'acierto', timestamp: Date.now() };
        const resultado = (0, scoring_service_1.calculateBaseScore)(evento, jugadorBase);
    ...
    it('debe sumar bonus por racha de aciertos', () => {
        const evento = { type: 'acierto', timestamp: Date.now(), streak: 3 };
        const resultado = (0, scoring_service_1.calculateBaseScore)(evento, jugadorBase);
        expect(resultado.puntos).toBe(100);
    ...
    it('debe restar puntos por fallo', () => {
        const evento = { type: 'fallo', timestamp: Date.now() };
        const resultado = (0, scoring_service_1.calculateBaseScore)(evento, jugadorBase);
    ...
    it('debe sumar puntos por participación', () => {
        const evento = { type: 'participacion', timestamp: Date.now() };
        const resultado = (0, scoring_service_1.calculateBaseScore)(evento, jugadorBase);
    ...
    it('debe sumar correctamente al tener puntos previos', () => {
        const jugador = Object.assign(Object.assign({}, jugadorBase), { puntosActuales: 50 });
        const evento = { type: 'acierto', timestamp: Date.now(), streak: 2 };
        const resultado = (0, scoring_service_1.calculateBaseScore)(evento, jugador);
        expect(resultado.total).toBe(100 + 20 + 50);
    });
    it('debe aplicar multiplicador x2 por respuesta <2s', () => {
        const evento = { type: 'acierto', timestamp: Date.now(), responseTimeMs: 1500 };
        const resultado = (0, scoring_service_1.calculateBaseScore)(evento, jugadorBase);
        expect(resultado.total).toBe(100 * 2);
    });
    it('debe aplicar multiplicador x1.5 por respuesta entre 2-5s', () => {
        const evento = { type: 'acierto', timestamp: Date.now(), responseTimeMs: 3000 };
        const resultado = (0, scoring_service_1.calculateBaseScore)(evento, jugadorBase);
        expect(resultado.total).toBe(100 * 1.5);
    });
    it('debe aplicar multiplicador x1.2 por respuesta entre 5-10s', () => {
        const evento = { type: 'acierto', timestamp: Date.now(), responseTimeMs: 7000 };
        const resultado = (0, scoring_service_1.calculateBaseScore)(evento, jugadorBase);
        expect(resultado.total).toBe(100 * 1.2);
    });
    it('debe aplicar multiplicador x1 por respuesta >10s', () => {
        const evento = { type: 'acierto', timestamp: Date.now(), responseTimeMs: 15000 };
        const resultado = (0, scoring_service_1.calculateBaseScore)(evento, jugadorBase);
        expect(resultado.total).toBe(100 * 1);
    });
    it('no debe aplicar multiplicador de velocidad a fallos', () => {
        const evento = { type: 'fallo', timestamp: Date.now(), responseTimeMs: 1000 };
        const resultado = (0, scoring_service_1.calculateBaseScore)(evento, jugadorBase);
        expect(resultado.total).toBe(-50);
    });
});
describe('calculatePlayerLevel', () => {
    it('debe asignar nivel 1 (Principiante) para score 0', () => {
        const nivel = (0, scoring_service_1.calculatePlayerLevel)(0);
    ...
    it('debe asignar nivel 2 (Aprendiz) para score 150', () => {
        const nivel = (0, scoring_service_1.calculatePlayerLevel)(150);
        expect(nivel.nivel).toBe(2);
        expect(nivel.nombre).toBe('Aprendiz');
        expect(nivel.puntosParaSiguiente).toBe(101);
    });
    it('debe asignar nivel 3 (Competidor) para score 300', () => {
        const nivel = (0, scoring_service_1.calculatePlayerLevel)(300);
    ...
    it('debe asignar nivel 4 (Experto) para score 800', () => {
        const nivel = (0, scoring_service_1.calculatePlayerLevel)(800);
        expect(nivel.nivel).toBe(4);
        expect(nivel.nombre).toBe('Experto');
        expect(nivel.puntosParaSiguiente).toBe(201);
    });
    it('debe asignar nivel 5 (Leyenda) para score 2000', () => {
        const nivel = (0, scoring_service_1.calculatePlayerLevel)(2000);
    ...
    it('debe asignar correctamente los puntos para siguiente nivel en el borde', () => {
        const nivel = (0, scoring_service_1.calculatePlayerLevel)(100);
        expect(nivel.nivel).toBe(1);
        expect(nivel.puntosParaSiguiente).toBe(1);
        const nivel2 = (0, scoring_service_1.calculatePlayerLevel)(101);
...
describe('generateLeaderboard', () => {
    ...
    it('debe ordenar por puntuación descendente', () => {
        const tabla = (0, scoring_service_1.generateLeaderboard)(jugadores);
    ...
    it('debe manejar lista vacía', () => {
        const tabla = (0, scoring_service_1.generateLeaderboard)([]);
...
describe('Historial de puntuaciones', () => {
    ...
    const now = Date.now();
    beforeEach(() => {
        scoring_service_1.scoreHistory.length = 0;
    });
    it('registra y recupera eventos correctamente', () => {
        const evento = { playerId, score: 100, level: 2, eventType: 'acierto', timestamp: now };
        (0, scoring_service_1.registerScoreEvent)(evento);
        const historial = (0, scoring_service_1.getPlayerHistory)(playerId);
        expect(historial.length).toBe(1);
        expect(historial[0]).toEqual(evento);
    });
    it('filtra por rango de fechas', () => {
        (0, scoring_service_1.registerScoreEvent)({ playerId, score: 50, level: 1, eventType: 'participacion', timestamp: now - 10000 });
        (0, scoring_service_1.registerScoreEvent)({ playerId, score: 100, level: 2, eventType: 'acierto', timestamp: now });
        const historial = (0, scoring_service_1.getPlayerHistory)(playerId, { from: now - 5000 });
        expect(historial.length).toBe(1);
        expect(historial[0].score).toBe(100);
    });
    it('filtra por tipo de evento', () => {
        (0, scoring_service_1.registerScoreEvent)({ playerId, score: 50, level: 1, eventType: 'participacion', timestamp: now });
        (0, scoring_service_1.registerScoreEvent)({ playerId, score: 100, level: 2, eventType: 'acierto', timestamp: now });
        const historial = (0, scoring_service_1.getPlayerHistory)(playerId, { eventTypes: ['acierto'] });
        expect(historial.length).toBe(1);
        expect(historial[0].eventType).toBe('acierto');
    });
    it('devuelve vacío si no hay eventos', () => {
        const historial = (0, scoring_service_1.getPlayerHistory)('otroJugador');
